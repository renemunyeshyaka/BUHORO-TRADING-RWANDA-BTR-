...
if(isset($_POST['delos']))
		{
			$rowid=$_POST['rowid'];
			$then=mysql_query("DELETE FROM `stouse` WHERE `Voucher`='$rowid' AND `Action`='REQUISE' LIMIT 100");
		}
...
if(isset($_POST['edits']))
		{
			$rowid=$_POST['rowid'];
			$then=mysql_query("UPDATE `stouse` SET `Voucher`='0' WHERE `Voucher`='$rowid' AND `Action`='REQUISE' LIMIT 100");
		}
...
// Close the current chart
		...
		{
			$dato=$_POST['dato'];
	$so=mysql_query("UPDATE `stouse` SET `Date`='$dato', `Voucher`='$vou' WHERE `Status`='0' AND `Voucher`='0' AND `Action`='REQUISE'");
		}
?>
...
$costo=number_format($cost, 2);
		...
		$dor=mysql_query("SELECT *FROM `stouse` WHERE `Status`='0' AND `Voucher`='0' AND `Action`='REQUISE' ORDER BY `Number` DESC");
				if($for=mysql_num_rows($dor)){
			  $click=0;
?>
 <div class="row">
            <div class="col-lg-12" style='margin-top:-20px;'>
             <span> &nbsp;&nbsp; Requisition Voucher No : <b><?php echo" $vou " ?></b></span> <span class="pull-right hidden-print"><?php echo $bra ?>&nbsp;&nbsp;<a href="#" onclick="return window.print()" class="hidden-print" title='Click to print' data-toggle='tooltip' data-placement='top'><i class="lnr lnr-printer"></i></a></span>
            <div class="widget-container fluid-height clearfix">
            <div class="widget-content padded clearfix">
	...
	$do=mysql_query("SELECT *FROM `items` WHERE `Status`='0' AND `Number`='$item' ORDER BY `Number` DESC");
		...
		print("<tr><form action='' method='post'>
			...
			<label class="control-label col-md-4"> </label>
				  ...
				  <form method='post' action=''><label class="control-label col-md-2"></label>
...
$tle="title='Save' data-toggle='tooltip' data-placement='top'";
	...
	?>
		...
		$dor=mysql_query("SELECT `stouse`.*, SUM(`Quantity`*`Price`) AS 'Tot', COUNT(DISTINCT(Item)) AS 'Ite' FROM `stouse` WHERE `Status`='0' AND `Voucher`!='0' AND `Action`='REQUISE' GROUP BY `Voucher` ORDER BY `Number` DESC LIMIT 15");
				if($for=mysql_num_rows($dor)){
?>
 <div class="row">
            <div class="col-lg-12" style='margin-top:-20px;'>
...
<table class="table table-striped table-hover">     
	...
	$n=1;			$tot=0;
		...
		print("<tr><td $stl class=hidden-xs><div align='center'>$code&nbsp;&nbsp;</td>
						<td class='text-center' $stl> $date </td><td $stl> $user </td><td class='text-center' $stl> $ite </td>
						<td $stl><div align='right'> $toto&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						<td $stl><div align='right'> 0.00&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						<td $stl><div align='right'> $toto&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>");
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      ...
      <div class='modal-footer' style='margin-top:-10px; height:50px; padding-top:5px; border:0px solid blue;'>
        ...
        <button type='$dbutn' name='delos' class='btn btn-sm btn-danger'>YES</button>
						...
						print("<form action='sition.php' method='post'><td class='hidden-xs hidden-print' align='right' style='width:40px; padding:0px;'>
                              <input type='hidden' name='rowid' value='$code'><input type='hidden' name='date' value='$date'>
							  <input type='hidden' name='p' value='0'><input type='hidden' name='pago' value='requit.php'>
							  <button type='submit' class='btn btn-xs btn-info hidden-print' name='opens' style='height:24px; padding:0px; margin:2px;' title='Open' data-toggle='tooltip' data-placement='top'>
					 ...
					 <thead>
					<tr><th class="hidden-xs"> </th><th colspan='2'><div align='center'> Grand Total </th>
					<th colspan='2'><div align='right'><?php echo $toto ?>&nbsp;&nbsp;&nbsp;</th>
					<th><div align='right'><?php echo $tos ?>&nbsp;&nbsp;&nbsp;</th>
					<th><div align='right'><?php echo $toto ?>&nbsp;&nbsp;&nbsp;</th>
					<th colspan='3' class="hidden-print"><div align='center'> -- </th></tr>
                  </table><br>
