<?php
if(basename($_SERVER['PHP_SELF']) == 'irepos.php') 
  $cm=" class='current'";
include'header.php';
  ...
  <ul class="list-group">   
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			 ...
			 <?php
			$dois=mysqli_query($conn, "SELECT `District` FROM `income` WHERE `District`!='' GROUP BY `District` ORDER BY `District` ASC");
			...
			<select class="form-control" name="custo">
			  ...
			  <?php
			$doi=mysqli_query($conn, "SELECT `vehicles`.`Plate`, `income`.`Vehicle` FROM `income` INNER JOIN `vehicles` ON `vehicles`.`Number` = `income`.`Vehicle` WHERE `vehicles`.`Trip`='0' GROUP BY `income`.`Vehicle` ORDER BY `income`.`Vehicle` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT `income`.* FROM `income` INNER JOIN `vehicles` ON `vehicles`.`Number` = `income`.`Vehicle` WHERE `income`.`Status`='0' AND `vehicles`.`Trip`='0' $conde $condi $cond ORDER BY `income`.`Date` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT `income`.* FROM `income` INNER JOIN `vehicles` ON `vehicles`.`Number` = `income`.`Vehicle` WHERE `income`.`Status`='0' AND `vehicles`.`Trip`='0' ORDER BY `income`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
$fo=mysqli_num_rows($do);
		 ...
		 <div class="row">
            ...
            <div class="widget-content padded clearfix">
			 ...
			 <table class="table table-striped table-hover" id="inco_Data">     
                                      <thead>
                    <tr role="row">
                     <th class="text-center"> No </th>
                       ...
                       <th class="text-center"> Voucher </th>
                        ...
                        <th class="text-center"> Distict </th>
...
$rate=$ro['Rate'];
$val=$amo*$rate;
$tam+=$amo;			$tdi+=$dista;	     	$twe+=$wei;	          $tva+=$val;
...
$doi=mysqli_query($conn, "SELECT *FROM `vehicles` WHERE `Number`='$emplo' AND `Trip`='0'");
			$roi=mysqli_fetch_assoc($doi);
				$fna=$roi['Plate'];
	
            $stn="style='padding:1px; font-size:11px;'";
           
	        print("<tr><td $stn class='text-center'>$n</td><td class='text-center' $stn> $dte </td><td $stn>&nbsp;$fna&nbsp;</td>
            <td class='text-center' $stn> $vou </td><td $stn> $driv </td>
            <td $stn> $garag </td><td $stn> $pur </td><td $stn> $descri </td>
            <td class='text-right' $stn> $weo&nbsp;TON </td>
			<td class='text-right' style='padding:1px; font-size:11px;'> $disto&nbsp;KM </td>
			<td class='text-right' style='padding:1px; font-size:11px;'> $amoo&nbsp;&nbsp;</td>
			<td class='text-right' style='padding:1px; font-size:11px;'> $rato&nbsp;&nbsp;</td>
		<td class='text-right' style='padding:1px; font-size:11px;'> $valo&nbsp;&nbsp;</td>");
...
$disa="";
    ...
    $disa="disabled";
		...
		<tr><th> </th><th colspan='7'>&nbsp;&nbsp;Total Amount </th>
