...
if(isset($_POST['delete_id']))
		{
			$rowid=$_POST['rowid'];
			$then=mysql_query("UPDATE `department` SET `Status`='1' WHERE `Depid`='$rowid' LIMIT 1");
		}
...
if(isset($_POST['search']))
		...
		if($searchkeyword)
			$conde="AND (`Depname` LIKE '%$searchkeyword%')";
		elseif($column_name)
			$conde="AND `Depname`='$column_name'";
		else
			$conde='';
		}


$do=mysql_query("SELECT *FROM `department` WHERE `Status`='0' $conde ORDER BY `Depid` ASC $cond");
...
*/
		...
		print("<tr>
                     ...
                     <form method=post action='depam.php'><td align='right' style='width:40px;padding:0px;'>
