...
if(isset($_POST['delo']))
		{
			...
			$thes=mysqli_query($conn, "DELETE FROM `consumption` WHERE `Invoice`='$rowid' AND `Purchase`='$swa' AND `External`='1' LIMIT 1");
		}
...
if(isset($_POST['delos']))
		{
			$rowid=$_POST['rowid'];
			$then=mysqli_query($conn, "DELETE FROM `stouse` WHERE `Voucher`='$rowid' AND `Action`='PURCHASE' LIMIT 100");
			
			
			$thes=mysqli_query($conn, "DELETE FROM `consumption` WHERE `Purchase`='$rowid' AND `External`='1' LIMIT 100");
		}
...
if(isset($_POST['edits']))
		{
			$rowid=$_POST['rowid'];
	$then=mysqli_query($conn, "UPDATE `stouse` SET `User`='$loge', `Voucher`='0', `Swap`='$rowid' WHERE `Voucher`='$rowid' AND `Action`='PURCHASE' LIMIT 100");
		}
...
if(isset($_POST['edit']))
		{
			...
			$pri=$_POST['pri'];
				$pri=str_replace(',', '', $pri);
			$curre=$_POST['curre'];
			$then=mysqli_query($conn, "UPDATE `stouse` SET `Quantity`='$qty', `Price`='$pri', `Rate`='$curre' WHERE `Number`='$rowid' LIMIT 1");
		}
...
// Add found item to the chart
		if(isset($_POST['addo']))
		{
			...
			$curre=$_POST['curre'];
				...
				if($qty)
	$so=mysqli_query($conn, "INSERT INTO `stouse` (`Date`, `User`, `Item`, `Quantity`, `Price`, `Destin`, `Action`, `Voucher`, `Status`, `Rate`) VALUES ('$Date', '$loge', '$plate', '$qty', '$pri', '', 'PURCHASE', '0', '0', '$curre')");
		}
	...
	$rece=mysqli_query($conn, "SELECT `Voucher` FROM `stouse` WHERE `Voucher`!='0' AND `Action`='PURCHASE' ORDER BY `Voucher` DESC LIMIT 1");
...
// Close the current chart
	...
	$so=mysqli_query($conn, "UPDATE `stouse` SET `Date`='$dato', `Voucher`='$vou', `Destin`='$supplier', `Invoice`='$refo' WHERE `Status`='0' AND `Voucher`='0' AND `Action`='PURCHASE' AND `User`='$loge'");
	
	$fd=mysqli_query($conn, "SELECT *FROM `stouse` WHERE `Voucher`='$vou' AND `Destin`='$supplier' AND `Date`='$dato'");
	...
	$doz=mysqli_query($conn, "SELECT *FROM `consumption` WHERE `Vehicle`='$plate' AND `Invoice`='$nume' AND `Purchase`='$vou' AND `External`='1' ORDER BY `Number` DESC");
	if($foz=mysqli_num_rows($doz))
	$so=mysqli_query($conn, "UPDATE `consumption` SET `Amount`='$amo', `Station`='$station', `Discount`='$disc', `Fuel`='$fuel', `Date`='$dato', `Price`='$pri', `Quantity`='$qty', `Rate`='$curr', `Point`='$refo' WHERE `Vehicle`='$plate' AND `Invoice`='$nume' AND `Purchase`='$vou' AND `External`='1'");
	else
	$so=mysqli_query($conn, "INSERT INTO `consumption` (`Vehicle`, `Amount`, `Station`, `Discount`, `Fuel`, `Date`, `Price`, `File`, `Invoice`, `Purchase`, `User`, `Quantity`, `Rate`, `External`, `Trip`, `Point`) VALUES ('$plate', '$amo', '$station', '$disc', '$fuel', '$dato', '$pri', '$newfilename1', '$nume', '$vou', '$loge', '$qty', '$curr', '1', '$tri', '$refo')");
	}
		}
...
// Delete the current chart
		...
		{
	$so=mysqli_query($conn, "DELETE FROM `stouse` WHERE `Status`='0' AND `Voucher`='0' AND `Action`='PURCHASE' AND `User`='$loge'");
		}
...
if(isset($_POST['addpa']))
	...
	$dof=mysqli_query($conn, "INSERT INTO `rpay` (`Date`, `User`, `Amount`, `Voucher`, `Action`, `Cheno`, `Rate`, `Pline`) VALUES ('$dati', '$loge', '$amo', '$code', '$count', '$refo', '$curr', '$pline')");
		...
		$dor=mysqli_query($conn, "SELECT *FROM `stouse` WHERE `Status`='0' AND `Voucher`='0' AND `Action`='PURCHASE' AND `User`='$loge' ORDER BY `Number` DESC");
		
			$_SESSION['Lin']="purcha.php";
		$_SESSION['Lim']="Purchase Orders";
?>
<div class="container-fluid main-content">
  ...
  <ul class="list-group">
    ...
    <li class="list-group-item">
	  ...
	  <a href="createsu.php">
                <p>
                <i class="lnr lnr-plus-circle"></i>&nbsp;Create Suppliers
                </p>
              </a></li>	
  ...
  <div class='modal-dialog' role='document'><div class='modal-content'><div class='modal-header text-left'>
    ...
    <label class='control-label' style='padding-top:10px;'>Vehicle ID</label>
        </div>
            <div class='col-md-6'>
<select class='form-control' name='plate' style='font-size:16px; height:34px;' required><option value=''> </option>";

$do=mysqli_query($conn, "SELECT `Number`, `Plate`, `Tripo` FROM `vehicles` WHERE `Status`!='1' AND `Plate` NOT LIKE '%GEN%' AND `Plate` NOT LIKE '%C2%' ORDER BY `Trip` DESC, `Plate` ASC");
		while($ro=mysqli_fetch_assoc($do)){
			$numb=$ro['Number'];
			$plate=$ro['Plate'];
			$tripo=$ro['Tripo'];
			
	$seco=mysqli_query($conn, "SELECT SUM(`Quantity`) AS 'Qty' FROM `consumption` WHERE `Vehicle`='$numb' AND `Trip`='$tripo' AND `Trip`!='0' ORDER BY `Number` DESC LIMIT 10");
			    $reco=mysqli_fetch_assoc($seco);
			    $qty=$reco['Qty'];
			    $qto=number_format($qty);
			    if($qto>0){
			    $un="L";
	if($qty>$flim AND !$_SESSION['Ofp'])
echo"<option value='' style='color:red;'> $plate &nbsp;&nbsp;&nbsp;&nbsp; $qto$un</option>";
    else
echo"<option value='$numb'> $plate &nbsp;&nbsp;&nbsp;&nbsp; 
<font style='color:blue;'>$qto$un</font></option>";
}
else{
echo"<option value='$numb'> $plate </option>"; 
	 ...
	 <div class='col-md-6'> &nbsp; </div>
		...
		<div class='row'> </div>
		 ...
		 <div class='form-group'>	 
			  <div class='col-md-5' align='right'>
			  <label class='control-label' style='padding-top:10px;'>Quantity (<font color='blue'>Liters</font>)</label></div>	
	 ...
	 <div class='col-md-3 col-xs-7'>
          <input name='pri' class='form-control text-center' type='text' onkeyup='format(this);' onkeypress='return isNumberKey(event)' required>
            </div><div class='col-md-3 col-xs-5' style='padding-left:0px; margin-left:0px;'>
	...
	<div class="col-lg-6" style="margin-right:0px; padding-right:0px;">	   
       ...
       <button class="btn  btn-warning btn-block" type="button" data-toggle='modal' data-target='#Modaldis' stle="font-size:13px;"><?php echo $af ?></button></div><div class="col-lg-2"> &nbsp; </div>
		...
		echo"<option value='' selected='selected'> SELECT SUPPLIER </option>";
			$top=mysqli_query($conn, "SELECT *FROM `suppliers` WHERE `Status`='0' ORDER BY `Supplier` ASC");
...
<table class="table table-striped table-hover">     
                    ...
                    <tr role="row">
                     <th> No </th>
                       ...
                       <th>&nbsp;&nbsp;Update&nbsp;&nbsp;</th>
                        ...
                        <th class="hidden-print" style="width:40px; text-align:center;" colspan='2'> Options </th>
                     </tr>
                    </thead>
                                        <tbody>
										<?php
	$n=1;			$tot=$sw=0;
			...
			if($swap!='0'){
	$doin=mysqli_query($conn, "UPDATE `stouse` SET `Swap`='$swap' WHERE `Status`='0' AND `Voucher`='0' AND `Action`='PURCHASE' AND `User`='$loge' ORDER BY `Number` DESC");
	        $swa=$swap;
	        $sw++;
			}
	...
	$do=mysqli_query($conn, "SELECT `Date` FROM `consumption` WHERE `Vehicle`='$item' AND `Date`<='$dte' ORDER BY `Date` DESC, `Number` DESC");
	...
	$arr="";            $to=$pri*$qt;				$too=number_format($to, 2);	
		...
		print("<tr><form action='' method='post'>
						...
						<td $stl> $iname </td><td $stl> FUEL CONSUMPTION </td><td $stl> PURCHASE </td>
						<td $stl><div align='center'> $uda </td><td $stl><div align='center'><input name='pri' class='form-control' type='text' onkeyup='format(this);' onkeypress='return isNumberKey(event)' onChange=this.style.color='#ff3366' style='text-align:right; width:100px; height:24px; margin:0px 0px 0px 0px;' value='$prio'></td>
		   ...
		   print("</select></td>
						...
						<td class='hidden-print' align='right' style='width:30px; padding:0px;'>
						  ...
						  <form method=post action=''><td class='hidden-print' align='right' style='width:30px; padding:0px;'>
                              <input type='hidden' name='rowid' value='$code'>
					 ...
					 <thead>
					<tr><th> </th><th colspan='4'><div align='center'> Grand Total </th>
	...
	<label class="control-label col-md-3"><input name="refo" class="form-control sm" type="text" placeholder="POINT OF FUELING" VALUE="<?php echo $refo ?>" onclick="return pageScroll()" OnKeyup="return cUpper(this);"></label>
  ...
  <div class='modal-dialog' role='document'>
	...
	?>
		...
		<?php
					    ...
					    if($supplier)
					        $conde="AND `Destin`='$supplier'";
				if($p=='0')
		$dor=mysqli_query($conn, "SELECT `stouse`.*, SUM(Quantity) AS 'Qty', COUNT(DISTINCT(Item)) AS 'Ite', SUM(Quantity*Price) AS 'Tot' FROM `stouse` WHERE `Status`='0' AND `Voucher`!='0' AND `Action`='PURCHASE' GROUP BY `Voucher` ORDER BY `Number` DESC LIMIT 20");
		        else
		$dor=mysqli_query($conn, "SELECT `stouse`.*, SUM(Quantity) AS 'Qty', COUNT(DISTINCT(Item)) AS 'Ite', SUM(Quantity*Price) AS 'Tot' FROM `stouse` WHERE `Status`='0' AND `Voucher`!='0' AND `Action`='PURCHASE' AND `Date` BETWEEN '$dato' AND '$datos' $conde GROUP BY `Voucher` ORDER BY `Number` DESC LIMIT 8000");
				if($for=mysqli_num_rows($dor)){
?>
 <div class="row">
            <div class="col-lg-12" style='margin-top:-20px;'>
...
<table class="table table-striped table-hover">     
                    ...
                    <tr role="row">
                     <th> No </th>
                       <th class='text-center'> Due&nbsp;Date </th>
                        <th> System&nbsp;User </th>
                       <th> Items </th><th> Supplier </th>
                       <th> Point of Fueling </th>
						<th class='text-center'> Quantity </th>
						<th class='text-center'> Amount </th>
						<th class='text-center'> Paid </th>
						<th class='text-center'> Balance </th>
                        <th class="hidden-print" style="width:40px; text-align:center;" colspan='4'> Options </th>
                     </tr>
                    </thead>
                                        <tbody>
										<?php
	$n=1;			$tqy=$tam=$tpa=$tba=0;
		...
		$qto=number_format($qty, 2);			$stl="style='padding:1px; font-size:12px;'";
	...
	<td class='text-center' $stl> $ite </td><td $stl> $dst </td><td $stl> $ref </td><td $stl><div align='right'> $qto </td><td $stl><div align='right'> $toto </td><td $stl><div align='right'>");
...
<td style='$sts'> $refe </td><td style='$sts' class='text-center'> $cur </td>
	  ...
	  <input type='hidden' name='supplier' value='$supplier'>
		...
		<button type='submit' class='btn btn-link text-danger hidden-print' style='height:16px; padding:0px; margin:0px; width:auto; margin-top:-5px;' name='pdelex'>&nbsp;&nbsp;<i class='lnr lnr-trash'></i>&nbsp;&nbsp;</button></div></td></form></tr>";
			$k++;
				}        
...
<h5 style='color:#ff0033'>Are you sure you want to delete this purchase?</h5>
      ...
      <div class='modal-footer' style='margin-top:-10px; height:50px; padding-top:5px; border:0px solid blue;'>
        ...
        <button type='$dbutn' name='delos' class='btn btn-sm btn-danger'>YES</button>
						...
						print("<form method=post action=''><td class='hidden-print' align='right' style='width:10px; padding:0px;'>
                              <input type='hidden' name='rowid' value='$code'><button type='submit' class='btn btn-xs btn-danger hidden-print' style='height:20px; padding:0px; margin:2px;' data-toggle='modal' data-target='$tags'>
						  &nbsp;&nbsp;<i class='lnr lnr-trash'></i>&nbsp;&nbsp;</button></td></form>
						  
		<td class='hidden-print' style='width:10px; padding:0px;'><div title='Pay' data-toggle='tooltip' data-placement='top'>
	<div align='center'>");
	
	if(!$_SESSION['Psi'])	
	echo"<button type='button' class='btn btn-xs btn-info hidden-print' style='height:20px; padding:0px; margin:2px;' data-toggle='modal' data-target='#exampleModalo$i'> &nbsp;&nbsp;<i class='lnr lnr-briefcase'></i>&nbsp;&nbsp; </button>";
...
$disa="";
	...
	<td class='hidden-print' align='right' style='width:10px; padding:0px;'>
        <input type='hidden' name='rowid' value='$code'><button type='submit' class='btn btn-xs btn-warning hidden-print' name='edits' style='height:20px; padding:0px; margin:1px;' title='Edit' data-toggle='tooltip' data-placement='top'> &nbsp;&nbsp;<i class='lnr lnr-pencil'></i>&nbsp;&nbsp;</button></td></form>
						  
			<form action='chase.php' method='post'><td class='hidden-print' align='right' style='width:10px; padding:0px;'>
                <input type='hidden' name='rowid' value='$code'>
                ...
                <input type='hidden' name='ite' value='$ite'>
					...
					<input type='hidden' name='pago' value='purcha.php'>
				<button type='submit' class='btn btn-xs btn-success hidden-print' name='opens' style='height:20px; padding:0px; margin:2px;' title='Open' data-toggle='tooltip' data-placement='top'>
