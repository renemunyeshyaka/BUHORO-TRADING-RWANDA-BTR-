...
$so=mysql_query("INSERT INTO `position` (`Postname`, `Starting`, `Done`, `Date`) VALUES ('$postna', '$starting', '$loge', '$Date')");
...
$pto=10;
		...
		{
			$rowid=$_POST['rowid'];
			$do=mysql_query("SELECT *FROM `position` WHERE `Postid`='$rowid' LIMIT 1");
...
*/
$so=mysql_query("UPDATE `position` SET `Postname`='$postna', `Starting`='$starting', `Done`='$loge' WHERE `Postid`='$rowid' LIMIT 1");
//$fo=mysql_num_rows($so);
$fo=1;

$do=mysql_query("SELECT *FROM `position` WHERE `Postid`='$rowid'");
				$ro=mysql_fetch_assoc($do);
					$code=$ro['Postid'];
					$pna=$ro['Postname'];
					$starting=$ro['Starting'];
...
<div class="page-title">
        <h1 style='margin-top:-20px; margin-bottom: 5px;'>Positions</h1>
...
<div class="col-md-2">
 ...
 <form method="post" class="form-horizontal" action="npost.php" enctype="multipart/form-data">
...
echo"<center><div class='alert alert-danger' style='width:48%;text-align:center;float:center;background-color: #60c560;color: #ffffff;border-radius:5px;'>
		<i class='lnr lnr-checkmark-circle'></i> &nbsp;&nbsp; <button class='close' data-dismiss='alert' type='button'>×</button>New Post Has Been Created.
		</div></center>";
if($pto==20)
echo"<center><div class='alert alert-danger' style='width:48%;text-align:center;float:center;background-color: #60c560;color: #ffffff;border-radius:5px;'>
		<i class='lnr lnr-checkmark-circle'></i> &nbsp;&nbsp; <button class='close' data-dismiss='alert' type='button'>×</button>Current Post Has Been Updated.
		</div></center>";
if($pto==30)
