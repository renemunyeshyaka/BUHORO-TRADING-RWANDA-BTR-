<?php
if(basename($_SERVER['PHP_SELF']) == 'rerepos.php') 
$cm=" class='current'";
include'header.php';
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo">
			 ...
			 <?php
			$doi=mysqli_query($conn, "SELECT `vehicles`.`Plate`, `repair`.`Vehicle` FROM `repair` INNER JOIN `vehicles` ON `vehicles`.`Number` = `repair`.`Vehicle` WHERE `vehicles`.`Trip`='0' GROUP BY `repair`.`Vehicle` ORDER BY `repair`.`Vehicle` ASC");
			while($roi=mysqli_fetch_assoc($doi)){
				$code=$roi['Vehicle'];
				$fna=$roi['Plate'];
				if($code==$custo)
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT `repair`.* FROM `repair` INNER JOIN `vehicles` ON `vehicles`.`Number` = `repair`.`Vehicle` WHERE `repair`.`Status`='0' AND `vehicles`.`Trip`='0' $conde $condi $cond ORDER BY `repair`.`Date` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT `repair`.* FROM `repair` INNER JOIN `vehicles` ON `vehicles`.`Number` = `repair`.`Vehicle` WHERE `repair`.`Status`='0' AND `vehicles`.`Trip`='0' ORDER BY `repair`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
$fo=mysqli_num_rows($do);
		 ...
		 <div class="row">
            <div class="col-lg-12">
			 ...
			 <span class="pull-right hidden-print">
			 &nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="exportTableToExcel('fuel_Data')" class="hidden-print" title='Export to excell' data-toggle='tooltip' data-placement='top'><i class="lnr lnr-line-spacing"></i></a></span>
            <div class="widget-container fluid-height clearfix">
            <div class="widget-content padded clearfix">

			 <table class="table table-striped table-hover" id="fuel_Data">     
                                      <thead>
                    <tr role="row">
...
$disa="";
    ...
    $disa="disabled";
			...
			print("</td><td style='$stl $clr'> $dati </td>
