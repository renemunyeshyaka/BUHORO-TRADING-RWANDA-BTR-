...
$date=$std; 
 while(strtotime($date) <= strtotime($end)){
	 ...
	 $doi=mysql_query("SELECT *FROM `payrolls` WHERE `Employee`='$empo' AND `Month`='$mti' AND `Year`='$yri' AND `D$ii`='1'");
		if($foi=mysql_num_rows($doi))
			$tot++;
...
<div class="container-fluid main-content" id="div_print" style='margin-top:-94px; border:0px solid blue;'>
 <div class='DONTPrint'>
<span onclick="return printpage()" class="print btn btn-primary pull-right" 
style="font-family: sans-serif; cursor: pointer; margin-top:10px; width:120px; margin-right:20px;">
<i class="lnr lnr-printer">&nbsp;Print</i></span>
</div>
...
<thead>
<tr><th width="40%">
<h4><strong>&nbsp;&nbsp;&nbsp;START_DATE</strong></h4></th>
<th width="30%"><h4><strong>END_DATE</strong></h4></th>
<th width="30%"><h4><strong><center>DAY(s)</strong></h4></th>
</tr>
</thead><tbody>
...
<td style="border: 0px;"><b>&nbsp;<?php echo $std ?></b></td>
 </tr>
</tbody>
</table>
...
<tbody>
  <tr>
<td style="border: 0px;"><b> </b></td>
<td style="border: 0px;"><b>&nbsp;<?php echo $end ?></b></td>
 </tr></tbody>
</table>
</td>
...
<tbody>
  <tr>
<td style="border: 0px;" align='center'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>&nbsp;<?php echo" $tot / $ndy "; ?> Day(s)</b></td>
<td style="border: 0px;"><b> &nbsp;&nbsp; </b></td>
 </tr>
 </tbody>
</table>
...
<table class="table table-striped invoice-table">
<thead>
<tr><td><table width='100%' class="table table-striped invoice-table "><thead><tr>
	<?php
	 $date=$std; 
 while(strtotime($date) <= strtotime($end)){
			 $pieces = explode("-", $date);
 ...
 while(strtotime($date) <= strtotime($end)){
			 $pieces = explode("-", $date);
				...
				$doi=mysql_query("SELECT *FROM `payrolls` WHERE `Employee`='$empo' AND `D$ii`='1' ORDER BY `Number` ASC");
		if($roi=mysql_num_rows($doi))
			$chk="checked";
		else
			$chk='';
						  echo"<th><div style='text-align:center'> <input type='checkbox' class='input-assumpte' id='input-confidencial' value='1' $chk> </th>";
								$date = strtotime("+1 day", strtotime("$date"));
				 $date=date("d-m-Y", $date);
					  }
...
<tr>
<th width="40%">
<h4><strong> MODE OF PAYMENT</strong></h4></th>
<th width="30%">
...
<td ALIGN='CENTER'><?php echo $dati ?></td>
<td><b><center>
