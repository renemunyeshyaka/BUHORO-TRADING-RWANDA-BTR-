<?php
if(basename($_SERVER['PHP_SELF']) == 'allrepo.php') 
  $cm=" class='current'";
include'header.php';
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo" style="margin-left:-55px; width:220px;">
			 ...
			 <?php
			$doi=mysql_query("SELECT `employees`.* FROM `allows` INNER JOIN `employees` ON `allows`.`Employee` = `employees`.`Eid` WHERE `employees`.`Status`='0' AND `employees`.`Eid`!='1001' GROUP BY `allows`.`Employee` ORDER BY `employees`.`Fname` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT *FROM `allows` WHERE `Status`='0' $conde $condi ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT *FROM `allows` WHERE `Status`='0' ORDER BY `Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
$fo=mysqli_num_rows($do);
					?>
					<div class="divFooter"><center><u><b>ALLOWANCE REPORT <?php echo"$mpri"; ?></b></u></center></div>
		 <div class="row">
            <div class="col-lg-12">
...
$stn="style='padding:1px;'";
		...
		print("<tr><td $stn class='text-right'>$n&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td $stn> $dte </td><td $stn> $fna $lna </td><td $stn> $purpo </td>
						<td class='text-right' style='padding:0px 40px 0px 0px;'> $amoo </td></tr>");
						  $n++;				$to+=$amo;
						}
	$to=number_format($to);
						?>
					...
					<th class="text-right" style='padding:0px 40px 0px 0px;'><?php echo $to ?></th></tr>
                  </table>
