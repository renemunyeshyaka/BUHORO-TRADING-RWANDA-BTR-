 <?php
	...
	$do=mysql_query("SELECT *FROM (SELECT *FROM `payrolls` WHERE `Date` BETWEEN '$dato' AND '$datos' GROUP BY `Month`, `Year` ORDER BY `Date` DESC LIMIT 12) SUB ORDER BY `Date` ASC");
					...
					elseif($p==2){
	$do=mysql_query("SELECT *FROM `payrolls` WHERE `Month`='$mt' AND `Year`='$yr' AND `Depart`='1' AND `Salary`<'40000' $conde ORDER BY `Currentp` ASC");
 ...
 <table class="table table-striped table-hover" style="font-size:8px;">
                    ...
                    <tr role="row">
                      <th style="padding:1px;font-size:10px;"> No </th> 					  
                       ...
                       <th style="padding:1px;font-size:10px;"> Net.Sal. </th>
                       <th style="padding:1px;font-size:10px;"> Other Income </th>	
                       <th style="padding:1px;font-size:10px;" class='text-center'> Total </th> 	
                       <th style="padding:1px;font-size:10px;" class='text-center'> Counted </th>  	
...
$neto=number_format($net);
	...
	<th colspan='3' class='text-right' $stn> $isalo </th>
...
$dot=mysql_query("SELECT *FROM `payrolls` WHERE `Month`='$mt' AND `Year`='$yr'");
 ...
 $amoo=number_format($amo);			$alloo=number_format($allo);			$advao=number_format($adva);				$deduo=number_format($dedu);
		...
		print("<tr><form action='handrepo.php' method='post'>
