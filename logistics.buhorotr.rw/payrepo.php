<?php
if(basename($_SERVER['PHP_SELF']) == 'payrepo.php') 
  $cm=" class='current'";
...
<li class="list-group-item">
	  <a href="rrepo.php">
                <p>
                <i class="lnr lnr-menu-circle"></i>&nbsp;Contribution Report
                </p>
              </a></li>   
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo" style="margin-left:-55px; width:220px;">
			 ...
			 <?php
		$doi=mysqli_query($conn, "SELECT `employees`.* FROM `payrolls` INNER JOIN `employees` ON `payrolls`.`Employee` = `employees`.`Eid` WHERE `employees`.`Status`='0' AND `employees`.`Eid`!='1001' GROUP BY `payrolls`.`Employee` ORDER BY `employees`.`Fname` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT *FROM `payrolls` WHERE `Payment`>'0' AND `Status`='0' $conde $condi ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT *FROM `payrolls` WHERE `Payment`>'0' AND `Status`='0' ORDER BY `Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
...
$dte=$ro['Date'];
$ado=$ro['Advance'];
	$adoo=number_format($ado);
$pay=$ro['Payment'];
	$payo=number_format($pay);
	
	$toto=number_format($pay+$ado);
...
$stn="style='padding:1px;'";
		...
		print("<tr><td $stn class='text-right'>$n&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td $stn> $dte </td><td $stn> $fna $lna </td><td $stn> $mt </td><td $stn> $yr </td>
						<td class='text-right' style='padding:0px 40px 0px 0px;'> $adoo </td>
						<td class='text-right' style='padding:0px 40px 0px 0px;'> $payo </td>
						<td class='text-right' style='padding:0px 40px 0px 0px;'> $toto </td></tr>");
						  $n++;				$tad+=$ado;						$tpa+=$pay;
						}
	$tot=number_format($tad+$tpa);						$tad=number_format($tad);					$tpa=number_format($tpa);					
						?>
						
                    </tbody>
					<tr><th> <th><th colspan='3'>&nbsp;&nbsp;&nbsp;&nbsp;Total Amount </th>
					<th class="text-right" style='padding:0px 40px 0px 0px;'><?php echo $tad ?></th>
					<th class="text-right" style='padding:0px 40px 0px 0px;'><?php echo $tpa ?></th>
					<th class="text-right" style='padding:0px 40px 0px 0px;'><?php echo $tot ?></th></tr>
                  </table>
