<?php
if(basename($_SERVER['PHP_SELF']) == 'insprepo.php') 
  $cm=" class='current'";
include'header.php';
  ...
  <ul class="list-group">   
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			 ...
			 <?php
	$dois=mysqli_query($conn, "SELECT `Place` FROM `inspection` WHERE `Place`!='' GROUP BY `Place` ORDER BY `Place` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT *FROM `inspection` WHERE `Status`='0' $conde $condi $cond ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT *FROM `inspection` WHERE `Status`='0' ORDER BY `Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
$fo=mysqli_num_rows($do);
					?>
					<div class="divFooter"><center><u><b>INSPECTION REPORT <?php echo"$mpri"; ?></b></u></center></div>
		 <div class="row">
            <div class="col-lg-12">
    ...
    <div class='modal-content'>
      ...
      <div class='modal-footer' style='margin-top:-10px; height:50px; padding-top:5px; border:0px solid blue;'>
        ...
        <button type='submit' name='delete_id' class='btn btn-sm btn-danger' $disa>YES</button>
				...
				if($file)
					echo"<a href='down_insp.php?link=$file'>
