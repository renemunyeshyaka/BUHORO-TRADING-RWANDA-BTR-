<?php
if(basename($_SERVER['PHP_SELF']) == 'insurepo.php') 
  $cm=" class='current'";
include'header.php';
...
if(isset($_POST['search']))
	...
	if(isset($_POST['delete_id']))
		{
			$rowid=$_POST['rowid'];
			$then=mysqli_query($conn, "DELETE FROM `insurance` WHERE `Number`='$rowid' LIMIT 1");
			$p=$_POST['p'];
			$dato=$_POST['dato'];
			$datos=$_POST['datos'];
			$custo=$_POST['custo'];
  ...
  <ul class="list-group">   
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			 ...
			 <?php
			$dois=mysqli_query($conn, "SELECT `Company` FROM `insurance` WHERE `Company`!='' GROUP BY `Company` ORDER BY `Company` ASC");
			...
			<select class="form-control" name="custo">
			 ...
			 <?php
			$doi=mysqli_query($conn, "SELECT `vehicles`.`Plate`, `insurance`.`Vehicle` FROM `insurance` INNER JOIN `vehicles` ON `vehicles`.`Number` = `insurance`.`Vehicle` GROUP BY `insurance`.`Vehicle` ORDER BY `insurance`.`Vehicle` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT *FROM `insurance` WHERE `Status`='0' $conde $condi $cond ORDER BY `Ending` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT *FROM `insurance` WHERE `Status`='0' ORDER BY `Date` DESC LIMIT 15) SUB ORDER BY `Ending` ASC");
$fo=mysqli_num_rows($do);
					?>
					<div class="divFooter"><center><u><b>INSURANCE REPORT <?php echo"$mpri"; ?></b></u></center></div>
		 <div class="row">
            <div class="col-lg-12">
...
if($end>=$Date AND $days<=10)
	...
	$stn="style='padding:1px;'";
					...
					print("<tr><td $stn class='text-center'>$n</td><td class='text-center' $stn> $dte </td>
						...
						<td class='text-center' $stn> $end </td><td class='text-center' $stn> $days $ds &nbsp;</td>
