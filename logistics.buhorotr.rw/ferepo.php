<?php
if(basename($_SERVER['PHP_SELF']) == 'ferepo.php') 
  $cm=" class='current'";
include'header.php';
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo">
			 ...
			 <?php
			$doi=mysqli_query($conn, "SELECT `vehicles`.`Plate`, `consumption`.`Vehicle` FROM `consumption` INNER JOIN `vehicles` ON `vehicles`.`Number` = `consumption`.`Vehicle` WHERE `vehicles`.`Trip`='1' AND `vehicles`.`Plate` NOT LIKE '%PETRO%' GROUP BY `consumption`.`Vehicle` ORDER BY `consumption`.`Vehicle` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT `consumption`.* FROM `consumption` INNER JOIN `vehicles` ON `vehicles`.`Number` = `consumption`.`Vehicle` WHERE  `consumption`.`Status`='0' AND `vehicles`.`Trip`='1' AND `vehicles`.`Plate` NOT LIKE '%PETRO%' $conde $condi $cond ORDER BY `consumption`.`Date` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT `consumption`.* FROM `consumption` INNER JOIN `vehicles` ON `vehicles`.`Number` = `consumption`.`Vehicle` WHERE `consumption`.`Status`='0' AND `vehicles`.`Trip`='1' AND `vehicles`.`Plate` NOT LIKE '%PETRO%' ORDER BY `consumption`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
...
$doi=mysqli_query($conn, "SELECT  `Plate`, `Driver` FROM `vehicles` WHERE `Number`='$emplo' AND `Trip`='1'");
...
$doil=mysqli_query($conn, "SELECT `Purchase` FROM `consumption` WHERE `Vehicle`='$emplo' AND `Purchase`!='0' AND `Purchase`<'$purcha' ORDER BY `Purchase` DESC LIMIT 1");
	if($foil=mysqli_num_rows($doil)){
        ...
        <td class='text-left' $stn> $dri</td><td class='text-right' $stn> $purcha</td>
						
    <td class='text-right' $stn> $qto</td>
	<td $stn>&nbsp;&nbsp;$fuel </td>
    ...
    <div class='modal-content'><div class='modal-header'>
      ...
      <div class='modal-header text-right;' style='height:50px; padding-top:10px; padding-right:60px; text-align:right;'>
		...
		print("<td class='text-right' $stn> $lpro </td><td class='text-right' $stn> $valo </td>");
...
$disa="";
	...
	$tva=number_format($tva, 2);				$tdi=number_format($tdi, 2);					$tsi=number_format($tsi, 2);
		...
		<tr><th colspan='4'>&nbsp;&nbsp;Total Amount </th>
			<th colspan='2' class='text-right' style='padding:0px'><?php echo $tq ?></th>
