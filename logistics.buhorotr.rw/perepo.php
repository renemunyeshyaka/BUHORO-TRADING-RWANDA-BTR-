<?php
if(basename($_SERVER['PHP_SELF']) == 'perepo.php') 
  $cm=" class='current'";
include'header.php';
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo">
			 ...
			 <?php
			$doi=mysqli_query($conn, "SELECT `vehicles`.`Plate`, `insurance`.`Vehicle` FROM `insurance` INNER JOIN `vehicles` ON `vehicles`.`Number` = `insurance`.`Vehicle` WHERE `vehicles`.`Trip`='1' GROUP BY `insurance`.`Vehicle` ORDER BY `insurance`.`Vehicle` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT `permit`.* FROM `permit` INNER JOIN `vehicles` ON `permit`.`Vehicle`=`vehicles`.`Number` WHERE `permit`.`Status`='0' $conde $condi $cond ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT `permit`.* FROM `permit` INNER JOIN `vehicles` ON `permit`.`Vehicle`=`vehicles`.`Number` WHERE `permit`.`Status`='0' ORDER BY `permit`.`Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
