<?php
if(basename($_SERVER['PHP_SELF']) == 'adlist.php') 
  $pr=" class='current'";
include'header.php';
...
if(isset($_POST['delete_id']))
		{
			$rowid=$_POST['rowid'];
			$then=mysqli_query($conn, "UPDATE `advance` SET `Status`='1' WHERE `Number`='$rowid' LIMIT 1");
			$p=$_POST['p'];
			$im=$_POST['im'];
			$file_to_delete = "advance/$im";
...
if(isset($_POST['search']))
	...
	if($p)
$do=mysqli_query($conn, "SELECT *FROM `advance` WHERE `Status`='0' $conde ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM `advance` WHERE `Status`='0' ORDER BY `Date` DESC $cond");
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      ...
      <div class='modal-footer' style='margin-top:-10px; height:50px; padding-top:5px; border:0px solid blue;'>
		...
		print("<tr><td $stn class='text-center hidden-xs'>$n</td>
						...
						<td class='text-right' style='padding:0px 40px 0px 0px;'> $balo </td>

                     <form method=post action='advance.php'><td align='right' style='width:20px; padding:0px;'>
                        <input type='hidden' name='rowid' value='$code'>
						  ...
						  &nbsp;&nbsp;<i class='lnr lnr-pencil'></i></td></form>
						   ...
						   <td align='right' style='width:20px; padding:0px;'>
			...
			$tav=number_format($tav);           $tpa=number_format($tpa);
			$tba=number_format($tba);
						print("<tr><th class='hidden-xs'> </th> 
	<th colspan='4' style='align:center; padding-left:80px;'> TOTAL AMOUNT </th>
	<th class='text-right' $stn> $tav </th><th class='text-right' $stn> $tpa </th>
	<th class='text-right' $stn> $tba </th>
	<th colspan='2' class='text-right hidden-print' $stn>  </th></tr>");
						?>
