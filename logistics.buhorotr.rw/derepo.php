<?php
if(basename($_SERVER['PHP_SELF']) == 'derepo.php') 
  $cm=" class='current'";
include'header.php';
    ...
    <form action="" method="post" class="form-horizontal "> 
        <div class="col-lg-10">
			...
			<select class="form-control" name="custo" style="margin-left:-55px; width:220px;">
			 ...
			 <?php
	$doi=mysqli_query($conn, "SELECT `employees`.* FROM `deduct` INNER JOIN `employees` ON `deduct`.`Employee` = `employees`.`Eid` WHERE `employees`.`Status`='0' AND `employees`.`Eid`!='1001' GROUP BY `deduct`.`Employee` ORDER BY `employees`.`Fname` ASC");
	  ...
	  <span class="input-group-addon"><i class="lnr lnr-calendar-full"></i></span>
             ...
             <?php
				if($p)
$do=mysqli_query($conn, "SELECT *FROM `deduct` WHERE `Status`='0' $conde $condi ORDER BY `Number` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM (SELECT *FROM `deduct` WHERE `Status`='0' ORDER BY `Date` DESC LIMIT 15) SUB ORDER BY `Date` ASC");
