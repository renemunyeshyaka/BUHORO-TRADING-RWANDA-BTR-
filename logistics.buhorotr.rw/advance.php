<?php
if(basename($_SERVER['PHP_SELF']) == 'advance.php') 
  $pr=" class='current'";
include'header.php';
...
if(isset($_POST['submit_duct']))
		{
			...
			$dato=$_POST['dato'];
			$pamo=$_POST['pamo'];
				$pamo=preg_replace('/,/', '', $pamo);

	$temp1 = explode(".", $_FILES["file1"]["name"]);
$newfilename1 = round(microtime(true)) . '.' . end($temp1);
move_uploaded_file($_FILES["file1"]["tmp_name"], "advance/" . $newfilename1);
	if(!end($temp1))
	$newfilename1='';

$so=mysqli_query($conn, "INSERT INTO `advance` (`Date`, `User`, `Amount`, `Purpose`, `Payment`, `Employee`, `Status`, `Balance`, `File1`) VALUES ('$dato', '$user', '$amo', '$purpo', '$pamo', '$emplo', '0', '$amo', '$newfilename1')");
		$pto=10;
		}
		...
		{
			$rowid=$_POST['rowid'];
			$do=mysqli_query($conn, "SELECT *FROM `advance` WHERE `Number`='$rowid' LIMIT 1");
				$ro=mysqli_fetch_assoc($do);
					...
					$user=$ro['User'];
					$dato=$ro['Date'];
					$pamo=number_format($ro['Payment']);

					$file1=$ro['File1'];
					if($file1)
						$dfile="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='down_adva.php?link=$file1'>Download File</a>";
					else
						$dfile="";
...
$so=mysqli_query($conn, "UPDATE `advance` SET `Employee`='$emplo', `Amount`='$amo', `Purpose`='$purpo', `Payment`='$pamo', `User`='$loge', `Date`='$dato' $upda WHERE `Number`='$rowid' LIMIT 1");
$pto=20;
...
<div class="col-md-2">
  ...
  <ul class="list-group">
	  ...
	  <a href="adlist.php">
                <p>
                <i class="lnr lnr-menu-circle"></i>&nbsp;Advances List
                </p>
              </a></li>  
...
echo"<center><div class='alert alert-danger' style='width:48%;text-align:center;float:center;background-color: #60c560;color: #ffffff;border-radius:5px;'>
		<i class='lnr lnr-checkmark-circle'></i> &nbsp;&nbsp; <button class='close' data-dismiss='alert' type='button'>×</button>Advance Record Has Been Created.
		</div></center>";
if($pto==20)
echo"<center><div class='alert alert-danger' style='width:48%;text-align:center;float:center;background-color: #60c560;color: #ffffff;border-radius:5px;'>
		<i class='lnr lnr-checkmark-circle'></i> &nbsp;&nbsp; <button class='close' data-dismiss='alert' type='button'>×</button>Advance Record Has Been Updated.
		</div></center>";
if($pto==30)
echo"<center><div class='alert alert-danger' style='width:48%;text-align:center;float:center;border-radius:5px;'>
 ...
 <div class="form-group">
  ...
  <div class="form-group">
			...
			<div class="col-md-2">
              <input name="pamo" class="form-control text-center" type="text" value="<?php echo $pamo ?>" onkeypress='return isNumberKey(event)' onkeyup='format(this);' required>
            </div> 
 </div>
