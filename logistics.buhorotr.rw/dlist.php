<?php
if(basename($_SERVER['PHP_SELF']) == 'dlist.php') 
  $pr=" class='current'";
include'header.php';
...
if(isset($_POST['delete_id']))
		{
			$rowid=$_POST['rowid'];
			$then=mysqli_query($conn, "UPDATE `deduct` SET `Status`='1' WHERE `Number`='$rowid' LIMIT 1");
			$p=$_POST['p'];
			$im=$_POST['im'];
			$file_to_delete = "deduct/$im";
...
if(isset($_POST['search']))
	...
	if($p)
$do=mysqli_query($conn, "SELECT *FROM `deduct` WHERE `Status`='0' $conde ORDER BY `Date` ASC");
	else
$do=mysqli_query($conn, "SELECT *FROM `deduct` WHERE `Status`='0' ORDER BY `Date` DESC $cond");
                     ...
                     <th class="hidden-xs"> No </th>
                       ...
                       <th> Employee`s&nbsp;Name</th>
                        <th class="hidden-xs"> Purpose/Issue </th> 
						 <th> Total&nbsp;Amount </th>
                        <th class='hidden-print' colspan='2' style="width: 10px;text-align:center;">Actions</th>
                     </tr>
                    </thead>
                                        <tbody>
					<?php
...
$doi=mysql_query("SELECT *FROM `employees` WHERE `Eid`='$emplo'");
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      ...
      <div class='modal-footer' style='margin-top:-10px; height:50px; padding-top:5px; border:0px solid blue;'>
		...
		print("<tr><td $stn class='text-center hidden-xs'>$n</td>
                     ...
                     <form method=post action='deduct.php'><td class='hidden-print' align='right' style='width:20px; padding:0px;'>
