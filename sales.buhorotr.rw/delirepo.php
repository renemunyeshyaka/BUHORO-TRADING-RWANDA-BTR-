	...
	if(isset($_POST['reprint']))
		{
			...
			$action='TAKEN';
			$cprint='DEL';
	$so=mysql_query("UPDATE `stouse` SET `Printed`='2' WHERE `Voucher`='$vou' AND `Action`='TAKEN' AND `Printed`='1' ORDER BY `Number` ASC LIMIT 100");
...
$doi=mysql_query("SELECT *FROM `stouse` WHERE `Date` BETWEEN '$dato' AND '$datos' AND `Status`='0' AND `Action`='TAKEN' AND `Voucher`!='0' AND `Destin`!='DAMAGED/EXPIRIES' $conde $condi GROUP BY `Voucher` ORDER BY `Voucher` ASC");
  ...
  <div class="col-lg-2 hidden-print">
   ...
   <ul class="list-group">
	...
	echo"<option value='$stonum' $s> &nbsp;&nbsp; $stona </option>";
				...
				?>
		</select></div><div class="col-lg-2 hidden-print"> 
					   
			   <select class="form-control" name="custo" style="padding-left:5px; padding-right:5px; width:190px; margin-left:-5px;">
			<?php

				echo"<option value='' selected='selected'> SELECT DESTINATION </option>";
			
	$dois=mysql_query("SELECT `Destin` FROM `stouse` WHERE `Action`='TAKEN' AND `Destin`!='' GROUP BY `Destin` ORDER BY `Destin` ASC");
...
$gto=0;
			   while($roi=mysql_fetch_assoc($doi)){
				$vous=$roi['Voucher'];
				$desto=$roi['Destin'];
	$do=mysql_query("SELECT *FROM `stouse` WHERE `Voucher` = '$vous' AND `Status`='0' AND `Action`='TAKEN' AND `Destin`='$desto'");
				?>
	...
	$tot=$qt*$pri;
		...
		print("<tr>
          <td class='hidden-xs' style='$stn'><div align='center'>$n&nbsp;&nbsp;</td>
			<td style='$stn'> $dte </td><td style='$stn'> $type </td><td style='$stn'> $pers </td><td style='$stn'> $tyso </td>
...
<td style='$stn'><div align='right'> $toto </td></tr>");
					...
					<?php
						...
						?>
    <button type='submit' class='btn btn-xs btn-success hidden-print' name='reprint' style='height:18px; padding:0px; margin:0px;' title='Reprint delivery voucher' data-toggle='tooltip' data-placement='top'>
		...
		if($_SESSION['Edit']){
			 $dbutn='submit';
			 $disa='Click to edit this delivery';
		 }
		 else{
			 $dbutn='button';
