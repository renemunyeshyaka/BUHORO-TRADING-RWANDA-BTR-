...
$tot=0;
$dore=mysql_query("SELECT *FROM `stouse` WHERE `Voucher`='$vous' AND `Action`='TRANSFER'");
while($rore=mysql_fetch_assoc($dore)){
 ...
 $dos=mysql_query("SELECT `Name` FROM `stores` WHERE `Store`='$dest' ORDER BY `Number` ASC");
		$ros=mysql_fetch_assoc($dos);
				$tsto=$ros['Name'];
...
?>
		...
		<textarea id="header" style='border: 1px solid #000000'>TRANSFER VOUCHER No <?php echo $vous ?></textarea>
		...
		<span style='float:right; text-align:right;'><b>DESTINATION:</b><?php echo"$fsto&nbsp;-&nbsp;$tsto"; ?>&nbsp;</span><br><br>
		...
		<TH width='15%'>TOTAL</TH>
		  ...
		  <?php
		  					$n=1;				$amo=0;
	$do=mysql_query("SELECT *FROM `stouse` WHERE `Voucher` = '$vous' AND `Status`='0' AND `Action`='TRANSFER'");
